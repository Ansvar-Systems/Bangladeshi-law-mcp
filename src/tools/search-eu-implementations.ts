/**
 * search_eu_implementations — Search EU directives/regulations referenced by Bangladeshi legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_bangladeshi_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  bangladeshi_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as bangladeshi_statute_count
  ...
  if (input.has_bangladeshi_implementation) {
    sql += ' HAVING bangladeshi_statute_count > 0';
  }
