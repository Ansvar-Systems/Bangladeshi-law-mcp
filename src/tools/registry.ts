/**
 * Tool registry for Bangladeshi Law MCP Server.
...
import { getBangladeshiImplementations, type GetBangladeshiImplementationsInput } from './get-bangladeshi-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Bangladesh Gazette. ' +
    ...
    'Call this FIRST when you need to understand what Bangladeshi legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Bangladeshi statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from a Bangladeshi statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate a Bangladeshi legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Bangladeshi statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Bangladeshi?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Bangladeshi legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether a Bangladeshi statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get international legal instruments that a Bangladeshi statute references or aligns with. ' +
      'Bangladeshi laws may reference international standards and conventions ' +
      'where applicable. ' +
      'Returns EU document identifiers, reference types, and implementation status.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Bangladeshi statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_bangladeshi_implementations',
    description:
      'Find all Bangladeshi statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Bangladeshi statutes. ' +
      'Bangladeshi may reference international legal instruments where relevant.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Bangladeshi legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_bangladeshi_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Bangladeshi legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within a Bangladeshi statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for a Bangladeshi statute or provision. ' +
      ...
      'Bangladeshi is an EU Member State; this checks implementation status and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Bangladeshi statute identifier.' },
...
const EU_TOOL_NAMES = new Set([
  'get_eu_basis',
  'get_bangladeshi_implementations',
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_bangladeshi_implementations':
          result = await getBangladeshiImplementations(db, args as unknown as GetBangladeshiImplementationsInput);
          break;
