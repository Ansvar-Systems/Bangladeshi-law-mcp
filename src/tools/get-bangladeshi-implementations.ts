/**
 * get_bangladeshi_implementations — Find Bangladeshi statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetBangladeshiImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface BangladeshiImplementationResult {
  document_id: string;
...
export async function getBangladeshiImplementations(
  db: InstanceType<typeof Database>,
  input: GetBangladeshiImplementationsInput,
): Promise<ToolResponse<BangladeshiImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as BangladeshiImplementationResult[];
