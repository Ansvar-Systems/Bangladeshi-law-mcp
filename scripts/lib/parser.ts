/**
 * HTML parser for Bangladeshi legislation from the Sejm ELI API (api.sejm.gov.pl).
 *
 ...
 *
 * Bangladeshi legislation references: Dz.U. YYYY poz. NNNN
 ...
 * to extract provisions from a Bangladeshi statute.
 *
 ...
 *   </div>
 */
export function parseBangladeshiHtml(html: string, act: ActIndexEntry): ParsedAct {
  ...
  for (let i = 0; i < articleStarts.length; i++) {
    ...
    // Bangladeshi acts use "ilekroć mowa" (whenever mentioned), "rozumie się przez to"
...
/**
 * Extract definitions from Bangladeshi legal text.
 *
 * Bangladeshi definitions typically use patterns like:
