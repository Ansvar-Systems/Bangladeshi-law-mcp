#!/usr/bin/env tsx
/**
 * Bangladeshi Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://bdlaws.minlaw.gov.bd';
const PORTAL_NAME = 'Bangladesh Laws (Ministry of Law)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/bangladeshi-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Bangladeshi Law MCP — Data Freshness Check');
